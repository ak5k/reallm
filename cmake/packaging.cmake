...
if(APPLE)
    ...
    if(
        ...
        AND NOT CODESIGN_INSTALLER_IDENTITY STREQUAL ""
    )
        set(CPACK_PRODUCTBUILD_IDENTITY_NAME "${CODESIGN_INSTALLER_IDENTITY}")
    endif()
endif()
...
if(
    APPLE
    AND CODESIGN_INSTALLER_IDENTITY
    AND NOT CODESIGN_INSTALLER_IDENTITY STREQUAL ""
    AND NOT TARGET notarize
)
    find_program(_xcrun_executable xcrun REQUIRED)

    set(_pkg_path "${CPACK_PACKAGE_DIRECTORY}/${_binary_pkg_file_name}.pkg")

    # notarytool credentials: prefer keychain profile, fall back to explicit
    # apple-id + team-id + app-specific password via environment.
    set(NOTARIZE_KEYCHAIN_PROFILE
        "$ENV{NOTARIZE_KEYCHAIN_PROFILE}"
        CACHE STRING
        "Keychain profile name for notarytool (xcrun notarytool store-credentials)."
    )

    if(NOTARIZE_KEYCHAIN_PROFILE)
        set(_notarize_credentials
            --keychain-profile
            "${NOTARIZE_KEYCHAIN_PROFILE}"
        )
    else()
        # Fallback: use explicit credentials from environment variables.
        # In CI, set these as repository secrets:
    ...
    add_custom_target(
        notarize
        COMMENT "Notarizing ${_binary_pkg_file_name}.pkg"
        COMMAND
            "${CMAKE_COMMAND}" -E echo
            "Submitting ${_pkg_path} to Apple notary service..."
        COMMAND
            "${_xcrun_executable}" notarytool submit "${_pkg_path}"
            ${_notarize_credentials} --wait
        COMMAND "${CMAKE_COMMAND}" -E echo "Stapling ticket to ${_pkg_path}..."
        COMMAND "${_xcrun_executable}" stapler staple "${_pkg_path}"
        VERBATIM
    )
endif()
