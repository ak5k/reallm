...
public:
  ...
  void prepare()
  {
    ...
    std::set_difference(safe.cbegin(), safe.cend(), unsafe.cbegin(),
                        unsafe.cend(), v.begin());
...
private:
  template <typename T> void vector_clean_duplicates_and_sort(std::vector<T>& v)
  {
...
public:
  ...
  Track(MediaTrack* tr) : tr{tr}, tr_g{GetTrackGUID(tr)}
  {
...
public:
  ...
  FX() : Track{}, tr{}, idx{}, g{}, tr_idx_{INT_MAX}
  {
  }

  FX(MediaTrack* tr, int fx_idx, bool local = false)
    : Track{tr}, tr{tr}, idx{fx_idx}, g{TrackFX_GetFXGUID(tr, fx_idx)},
      tr_idx_{INT_MAX}
  {
    if (!local)
    {
...
public:
  ...
  FXExt() : FX{}, enabled{}, pdc{}, inst{false}
  {
  }

  FXExt(MediaTrack* tr, int fx_idx)
    : FX{tr, fx_idx, true}, enabled{TrackFX_GetEnabled(tr, fx_idx)}, pdc{},
      inst{false}
  {
