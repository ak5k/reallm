...
std::vector<T> Network<T, U, V>::get_neighborhood(T& k)
{
  ...
  for (auto i = 0; i < num_sends; i++)
  {
    ...
    neighbor =
      (MediaTrack*)(uintptr_t)GetTrackSendInfo_Value(k, 0, i, "P_DESTTRACK");
    if (!mute)
    {
      v.push_back(neighbor);
    }
...
V Network<T, U, V>::analyze(T& k, U& r, V v)
{
  ...
  for (auto i = 0; i < fx_count; i++)
  {
    ...
    else if (find(fx_safe.cbegin(), fx_safe.cend(), guid) != fx_safe.cend())
    {
      safe = true;
    }
    ...
    if (pdc > 0)
    {
      ...
      {
        pdc_temp = pdc_temp - pdc;
        auto k = find(fx_to_disable.cbegin(), fx_to_disable.cend(), guid);
...
static void initialize(vector<MediaTrack*>& v)
{
  ...
  for (auto i = 0; i < GetNumTracks() + 1; i++)
  {
    ...
    if ((flags & 64 && (flags & 128 || flags & 256)) ||
        (global_automation_override > 1 && global_automation_override < 6) ||
...
static bool process_fx(FXState& fxstate)
{
      ...
      !tr_pdc_to_enable.empty() || !tr_pdc_to_disable.empty())
  {
    auto preventCount = fx_to_disable.size() + fx_to_enable.size() +
                        tr_pdc_to_enable.size() + tr_pdc_to_disable.size() + 4;
    ...
    for (auto&& i : fx_to_enable)
    {
      if (fx_map.find(i) != fx_map.end())
      {
        ...
        if (ValidatePtr2(0, fx.tr, "MediaTrack*"))
        {
          ...
          string s = fx.name;
          eraseSubStr(s, "llm: ");
          TrackFX_SetNamedConfigParm(fx.tr, fx.idx, "renamed_name", s.c_str());
        }
      }
    }
          ...
          if (!param_change.empty())
          {
            ...
            for (auto i = 0; i < fx_count; i++)
            {
              ...
              if (param_change.find(fx.name) != param_change.end())
              {
                ...
                {
                  TrackFX_SetParam(fx.tr, fx.idx, i.first, i.second.second);
                }
              }
            }
          }
          ...
          if (!param_change.empty())
          {
            ...
            for (auto i = 0; i < fx_count; i++)
            {
              ...
              if (param_change.find(fx.name) != param_change.end())
              {
                for (auto&& i : param_change[fx.name])
                {
                  TrackFX_SetParam(fx.tr, fx.idx, i.first, i.second.first);
                }
              }
            }
          }
...
static void Get(const char* parmname, char* buf = nullptr, int bufSz = 0,
                MediaTrack* tr = nullptr)
{
      ...
      strcmp(parmname, "VECTOR") == 0)
  {
    ...
    for (auto&& i : routes)
    {
      for (auto&& j : i)
      {
        auto trNum = (int)GetMediaTrackInfo_Value(j, "IP_TRACKNUMBER") - 1;
        if (trNum < -1)
        {
          trNum = -1;
        }
        ...
        if (strcmp(parmname, "VECTOR") != 0)
        {
          for (auto k = 0; k < TrackFX_GetCount(j); k++)
          {
            ...
            auto v = find(fx_disabled.cbegin(), fx_disabled.cend(), g);
  ...
  else if (strcmp(parmname, "GRAPH") == 0)
  {
    ...
    for (auto&& i : network)
    {
      auto trNum = (int)GetMediaTrackInfo_Value(i.first, "IP_TRACKNUMBER") - 1;
      if (trNum < -1)
      {
        trNum = -1;
      }
    ...
    for (auto&& i : param_change)
    {
      for (auto&& j : i.second)
      {
        ...
        s += i.first + "," + to_string(j.first) + "," +
             to_string(j.second.first) + "," + to_string(j.second.second);
      }
    }
  }
...
void Register(bool load)
{
  ...
  custom_action_register_t action{
    0, "AK5K_REALLM", "ReaLlm: REAPER Low latency monitoring", nullptr};
  ...
  if (load)
  {
    ...
    plugin_register("APIvararg_Llm_Get",
    ...
    plugin_register("APIvararg_Llm_Set",
