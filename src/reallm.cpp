...
class TrackFx
{
  public:
    ...
    TrackFx(GUID *g, MediaTrack *tr, int fx_index) : g(g), tr(tr), fx_index(fx_index)
    {
    }
...
std::vector<MediaTrack *> GetAllTrackSendDestinations(MediaTrack *sourceTrack)
{
    ...
    {
        MediaTrack *destinationTrack = (MediaTrack *)(UINT_PTR)GetTrackSendInfo_Value(sourceTrack, 0, j, "P_DESTTRACK");
...
int CalculateTrackPdc(MediaTrack *tr, int initial_pdc, std::unordered_set<TrackFx *> &fx_set)
{
    ...
    for (int j = 0; j < fx_count; j++)
    {
        ...
        else if (fx_set_prev.find(&fx_map[g]) != fx_set_prev.end() && !fx_map[g].getSafe() && TrackFX_GetEnabled(tr, i))
        {
...
void main()
{
    ...
    SetProjExtState(0, "ak5k", "reallm", serializeFxSet(fx_set_to_disable).c_str());
